...
**yaw rotation** [cite that this is not part of motion primitive]
...
Algorithm 1 shows pseudocode for a naive approach which is a precursor to the greedy approach discussed later. Note that this algorithm ignores rotation constraints. The algorithm generates trajectory segments (motion primitives) $\tau_i$ for every $i \in \{2, \dots, n\}$. Segment $\tau_i$ is the trajectory between constraints $C_{i-1}$ and $C_i$. Lines 1 to 3 set initial conditions so that the first trajectory segment will have an initial position of $x_i$, an initial velocity of $(0, 0, 0)$, and an initial acceleration of $(0, 0, 0)$. The algorithm proceeds by computing $\tau_i$ from $\tau_{i-1}$. $p_0$, $v_0$, and $a_0$ are the initial position, velocity, and acceleration, respectively, of $\tau_i$. For the trajectory to be a continuous function, these must be the final position, velocity, and acceleration, respectively, of $\tau_{i-1}$. $p_1$, $v_1$, and $a_1$ are the final position, velocity, and acceleration of $\tau_i$. The final position for this trajectory segment must be the user supplied position constraint corresponding to the end of the segment, which is $x_i$. The final velocity and acceleration of $\tau_i$ are left unconstrained, which is the defining feature of this naive approach. The duration of this trajectory segment must be the user supplied duration constraint, which is $t_i$. Finally, the trajectory segment $\tau_i$ is generated with the appropriate arguments.
